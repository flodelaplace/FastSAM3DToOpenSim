...
"""
Fast SAM 3D Body — Avatar-only generation
==========================================
...
Output dir contient :
  ...
  markers_<name>_avatar_<template>.glb  — 1 par template auto-détecté
...
def main(args):
    ...
    # Avatar pipeline doesn't need the raw per-frame outputs JSON.
        ...
        for tpl in templates:
            ...
            for tag in ("_apose_opaque", "_opaque", "_apose"):
                if short.endswith(tag):
                    short = short[: -len(tag)]
                    break
            avatar_out = os.path.join(args.output_dir, f"{prefix}_avatar_{short}.glb")
            try:
                ...
                print(f"    ✓ {short:24s} → {avatar_out}")
            except Exception as e:
                print(f"    ✗ {short:24s} FAILED: {e}")
